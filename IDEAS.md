...
### I1. Carry vs Reset (기반 2×2 ablation) — `완료: 가설 기각 (라운드 1, s95)`
- **가설:** fast-weight state를 loop 간 carry(= inner epoch 추가)하면 reset(naive, attention식 loop)보다 낫다.
- **결과:** reset 22.204 > carry 21.751 (−0.45dB). **naive carry는 해로움** — core challenge 실증.
  단, reset조차 L8을 +0.249dB 역전 (RQ1 강한 yes). → fix 계열(I2~I4)이 carry를 구제하는지가 r2의 질문.

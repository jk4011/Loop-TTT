...
## WAVE 21 (미니멀 시대 — sup 제외 확정, 2026-07-21)

사용자 확정: sup(per-loop 감독)도 제외 — ep2/KD/sup 없이. 방법론 = loop + per-loop affine 조건화
(film·gates·shift·nlcond) + optzone(wd=0, lr×64). 추가 파라미터 <1%, FLOPs ~0, task-agnostic.

큐 (전부 no-sup, 자동 투입 대기):
1. r21_gfna_lr64_s95 ★ — 헤드라인 후보: full-affine 번들(boost 없음, sup 없음)
2. r21_film_lr64_s95 — 단일 다이얼 미니멀 (film만)
3. r21_gfn_lr64_s95 — affine 제외 (shift 기여 attribution)
4. r21_gfna_lr128_s95 — no-sup 체제 lr 스윕
이후: 승자 3-seed → 최종 헤드라인.

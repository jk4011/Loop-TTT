...
| ours (+다이얼@lr8) | 1.002× | 4× | 21.05 |
| ours (+다이얼+inner full@lr16) | 1.003× | 4× | 20.85 |
| 12층 원본 (iso-compute) | 2.4× | 4× | 18.40 |

- **inner-affine은 3B/d768 대규모서 3다이얼 위에 이득 없음** (W7, node2): 다이얼+inner full 20.85 ≈
  다이얼만 20.854. 16M/WikiText에선 inner가 −3.94(affine_inner) 도왔으나(W5) 대규모서 소멸 →
  **inner-affine "사이사이" 이득은 스케일 의존(소형 유효, 대형 무효); outer 3다이얼은 스케일 불변**.
  대형 실배포 미니멀 처방으로는 3다이얼로 충분함을 시사(inner 사이트 오버헤드 불필요).

- **3B loop-param lr 정점 = lr16** (W3, node2): lr16 20.85 < lr8 21.05 → 3B에서도 lr16이 lr8보다
  우수(−0.20 ppl). NVS는 스케일업 시 정점이 lr128 쪽으로 이동했으나(위 d512 절), LM 3B는 lr16 유지 —
  태스크·아키텍처별 loop-param lr 정점이 다름을 재확인.
...
| +2다이얼 @lr128 (paired ΔPSNR) | — | +0.582 | +0.646 | **+0.614** (2-seed) | −0.014 |
...
- **d512 lr 정점 (W6, 2-seed s96/s97)**: lr128 avg +0.614 > lr64 동일-seed avg +0.563 (+0.05) →
  d512에선 정점이 lr128 쪽으로 약간 이동(d256에선 64≈128였음). 스케일업 시 최적 loop-param lr이
  커지는 경향 — 단 이득 폭은 작아 lr64도 실용적으로 충분.
...
| LayerScale per-loop | −1.609 | **+0.080** (t=8.3) | **+1.69** | 처방이 참사→미약한 양성까지 회복하나 scale-단독 형태는 이득 작음 |
...
- **adaLN optzone 3-seed 확정 (W6)**: +0.620 (s95) / +0.598 (s96) / +0.383 (s97) → **평균 +0.534**,
  3/3 유의(t=19~33). seed 간 편차는 있으나 전부 강양성 — "adaLN 형태는 optzone 처방으로 살아난다"가
  단일-seed 우연이 아님을 확인(우리 gates+film +0.569에 근접).
- **분해 완성 (3/3)**: 회복폭 DVLT +0.08 / LayerScale +1.69 / adaLN +1.48 → **처방(optzone)은 전
  형태에 필요조건**. 처방 후 순위 adaLN(+0.62) ≈ ours(+0.57) ≫ LayerScale(+0.08) > DVLT(0) →
  **형태 요건 = shift 포함(scale-단독 부족: LayerScale·DVLT) + 입력측(A-site) 변조 포함(adaLN·ours)**.
  이 요건을 갖추면 생성형(adaLN MLP)이냐 직접 테이블(ours)이냐는 부차적 — 단 ours는 파라미터
  1/40 수준(테이블 vs 임베딩+MLP)으로 같은 효과 → 미니멀 주장 유지.

#### d512 다이얼 lr 정점 (node1, 2026-07-22)

| d512 gf (s95) | dPSNR | LPIPS |
|---|---|---|
| lr×16 | +0.297(s96/97 평균과 함께 3-seed +0.287) | — |
| lr×64 | +0.637 | 0.2078 |
| **lr×128** | **+0.703** | **0.2017** |

lr128이 s95에서 +0.066 우위(단일 seed, 노이즈 경계) — s96/s97 승격 큐잉(W6). d256에서도 128≈64였음
→ NVS는 스케일을 키워도 다이얼 고lr 선호 유지(LM의 스케일 역비례와 대조).

### inner-affine "사이사이" — LM 검증 (W5, node2, 2026-07-22)

공유 행렬 사이 **모든 이음새**(attn: to_qkv 직후·c_proj 직전, TTT 동일, MLP: gelu 은닉)에 loop별
zero-init affine을 넣는 사용자 아이디어. 16M/WikiText103, 12k steps, s95, optzone lr×64. (ppl↓ 우수)

| 구성 (16M) | ppl (s95) | ppl (s96) | 2-seed 평균 | vs 3다이얼(59.14) |
|---|---|---|---|---|
| naive loop 2×4 | 74.45 | — | — | — |
| 3다이얼 (기존 최고) | 59.14 | — | — | — |
| **inner_only** (inner만, 다이얼 없음) | 57.68 | 57.13 | **57.41** | **−1.73** |
| **3다이얼 + inner full** (affine_inner) | 55.20 | 55.42 | **55.31** | **−3.83** |
| 3다이얼 + TTT-이음새 inner만 (innerttt) | 59.45 | — | — | +0.31 |

(seed 승격 W6: inner_only 57.68/57.13, affine_inner 55.20/55.42 — 둘 다 seed 간 ±0.3 이내로
안정, "inner 단독 > 3다이얼", "스택 최상" 결론이 2-seed에서 유지.)

- **inner-affine 단독이 outer 3다이얼을 이긴다** (57.68 < 59.14): "사이사이" 조건화가 LN 밖·행렬
  사이에 앉아 같은 가중치를 loop마다 진짜 다른 연산자로 합성 — LN이 흡수 못 하는 자유도.
...
**gf+inner full NVS 3-seed 확정 (W8)**: +0.660 (s95) / +0.551 (s96) / +0.634 (s97) → **평균 +0.615**,
3/3 유의(t 25~32). NVS 최고 스택이 seed 우연 아님 — inner-affine이 2다이얼 위에 안정적으로 가산.
...
#### L1×8 극한 케이스 — "다이얼 = 잃어버린 layer-다양성의 압축 복원" (W11, LM, s95)

고유 layer를 **1개**로 줄이고 8회 loop(iso-compute 유효깊이 8 유지). depth 다양성이 전부 다이얼 몫이
되므로, 다이얼 이득이 L2×4보다 커야 가설이 성립. (16M, WikiText, ppl↓)

| 구성 | L1×8 ppl | L1×8 회복 | (참고) L2×4 ppl | L2×4 회복 |
|---|---|---|---|---|
| naive loop | 86.73 | — | 74.45 | — |
| +3다이얼 | 64.02 | **−22.71** | 59.14 | −15.31 |
| +3다이얼+qkv-inner | **61.43** | **−25.30** | 55.81 | −18.64 |

- **다이얼 회복이 L1×8에서 확연히 큼** (−22.71/−25.30 vs L2×4의 −15.31/−18.64): 고유 layer가 1개뿐일
  때 naive가 −12.28 더 나쁜데(86.73 vs 74.45), 다이얼이 그 손실의 대부분을 되메움 →
  **"다이얼은 weight-tying으로 잃은 layer-다양성을 파라미터로 복원한다"는 서사를 정량 지지.**
- 단 절대 ppl은 L1×8(61.43)이 L2×4(55.81)를 못 따라잡음 — 다이얼은 다양성 손실을 크게 줄이나
  완전 복원은 아님. qkv-inner가 3다이얼 위에 L1×8에서도 −2.59 추가(사이사이 이득 재확인).

**NVS L1×8 (W11, d256, s95, paired vs naive L1×8 22.007):**

| 구성 | L1×8 ΔPSNR | (참고) L2×4 ΔPSNR | inner-affine 한계기여 |
|---|---|---|---|
| naive loop | — (22.007) | — (22.204) | — |
| +2다이얼 (gf) | +0.484 (t=26.8) | +0.569 | — |
| +2다이얼+qkv-inner | **+0.764** (t=30.6) | +0.646 | L1×8 **+0.280** vs L2×4 +0.077 |

L1×8 gf 3-seed 확정 (W15, node2): +0.484 (s95) / +0.451 (s96) / +0.608 (s97) → **평균 +0.514**, 3/3
유의(t 22~30). (inner 가산 gfqkv−gf의 3-seed는 node1 gfqkv s96/s97 합류 후 확정 — s95 단독 +0.280.)

- **inner-affine이 극한 tying에서 특히 빛난다**: 2다이얼(outer FiLM/게이트)은 L1×8에서 오히려 이득이
  작지만(+0.484<+0.569), **qkv-inner(행렬-사이 affine)는 L1×8에서 +0.280 더함 — L2×4(+0.077)의 3.6배.**
  전체 스택은 L1×8(+0.764)이 L2×4(+0.646)를 상회.
- **양 태스크 수렴**: LM(다이얼 회복 L1×8이 큼)과 NVS(inner-affine 기여 L1×8이 큼) 모두 "weight-tying으로
  잃은 layer-다양성을 loop별 조건화가 복원"을 지지. 단 NVS는 naive L1×8 자체가 L2×4와 −0.20뿐이라
  (다양성 손실이 작아) 복원 몫을 주로 inner-affine이, outer 다이얼은 오히려 슬롯이 8개로 늘어 희석.
...
### W4b — NVS inner 사이트 분해 완성 (d256, s95, lr64, 2026-07-22)

| 구성 | dPSNR | inner의 가산 (vs gf +0.569) |
|---|---|---|
| gf(2다이얼) | +0.569 | — |
| gf + out측만 (attn+TTT c_proj직전) | +0.559 | **−0.010 (무효)** |
| gf + TTT 이음새만 | +0.627 | +0.058 |
| gf + MLP 은닉만 | +0.635 | +0.066 |
| gf + attn 이음새만 | +0.640 | +0.071 |
| gf + qkv측만 (attn+TTT) | **+0.646** | **+0.077 (단일사이트 최강)** |
| gf + inner full | **+0.660** | +0.091 |
| (참고) inner만, 다이얼 없음 | +0.320 | — |

1. **입력측이 답, 출력측은 무효**: qkv측 +0.077 vs out측 −0.010. W2 형태요건("입력측 변조 필수,
   출력 scale만으론 부족")이 inner 사이트 수준에서 재등장 — 같은 원리의 프랙탈 반복.
2. **사이트 중복성(sub-additive)**: 단일사이트들이 각각 +0.06~0.08인데 full은 +0.091 — 합산(+0.27)의
   1/3. 아무 입력측 이음새 하나가 inner 가산분의 대부분을 담음 → 용량이 아니라 "loop별 재표현 경로
   1개 확보"가 본질.
3. **LM의 "TTT-이음새 무효"는 NVS에서 재현 안 됨**: NVS TTT-이음새 +0.058 ≈ attn +0.071 (LM에선
   innerttt가 0). 사이트 특이성은 태스크 의존, "입력측>출력측"과 "스택 최상"은 태스크 공통.
4. **미니멀 후보**: gf + qkv측만(+0.646)이 full(+0.660)과 노이즈 수준 차이 — 사이트 5→2로 줄여
   오버헤드도 절감. seed 승격 진행 중 (r24_gf_inner_qkv s96/s97, node1).

### 오버헤드 벤치 — compile이 다이얼 비용을 회수 (node2, 단독 GPU, 600스텝, 2026-07-22)

| 구성 (d256) | eager it/s | compile it/s | eager Δ | compile Δ |
|---|---|---|---|---|
| naive | 7.40 | 15.33 | — | — |
| gf | 6.58 | 14.15 | −11.1% | **−7.7%** |
| gf+inner full | 5.12 | 13.96 | **−30.8%** | **−9.0%** |

- **compile이 inner 오버헤드를 −31%→−9%로 회수**하고 절대 처리량 ~2배.
- **정정 (2026-07-23, 사용자 지적)**: 앞 절의 공유-GPU 로그 기반 수치(다이얼 −5~10%, inner −13~15%)는
  **과소측정** — (a) 동시 런들의 CPU/dataloader 경합이 naive 분모까지 부풀리고, (b) 입력대기 틈새에
  다이얼 커널이 겹쳐 실행되어 절대 추가비용 자체가 가려지며(+41ms로 보였으나 실제 +60ms), (c) inner
  수치는 교차노드 비교였음. **유효 수치는 본 절의 단독-GPU 벤치**: step-time 기준 eager gf +12.5% /
  gf+inner +45%, compile gf +8.3% / gf+inner +9.8% (절대 +5.4/+6.4ms). 실사용 기준은 compile 열.
- 실사용 처방: **compile 필수, 그 위에서 다이얼+inner 비용은 한 자릿수 %** (naive도 같이 2배 빨라지는
  건 양쪽 공통이라 상대 비용만 남음). + 사이트 프루닝(qkv만, 위 W4b-4)으로 추가 절감 여지.
- 지금까지의 본실험은 전부 eager (naive·ours 동일 조건 — 공정성 유지).

### 3B lr 정점 확정 (W3, node2): lr8 21.05 > lr16 20.854 → **3B 다이얼 정점은 lr16** (0.5B lr16과 동일).

### LM qkv-사이트 검증 (W9, node1, 2026-07-22): **양 태스크 수렴 확정**

| LM 16M (s95) | ppl |
|---|---|
| 3다이얼 | 59.14 |
| 3다이얼 + inner full (5사이트) | 55.20 |
| **3다이얼 + qkv측만 (2사이트)** | **55.81** (s96 55.79) |

미니멀 처방 2-seed 확정 (W12, node2): qkv-inner s95 55.81 / s96 55.79 → 평균 55.80, ±0.02로 매우
안정. full inner(55.20)에 근접하며 seed 우연 아님.

NVS(qkv +0.646 ≈ full +0.660)에 이어 LM에서도 **qkv측 2사이트가 full 5사이트 이득의 ~85%**를 담음
(59.14→55.20 중 3.33/3.94). → **교차태스크 미니멀 처방: "다이얼 + qkv측 inner"** — 사이트 5→2로
오버헤드도 절감. (LM은 TTT-이음새 단독 무효였지만 qkv측(attn+TTT 공용)은 유효 — 유효성분이 attn
qkv에 있음을 시사, NVS는 사이트 무차별.)

### W10 — 절대 처리량, 알맞은 baseline 포함 (node1 gpu0 단일세션, d256, 600스텝, 2026-07-23)

| 모델 (d256) | params | eager it/s | compile it/s | compile 처리량 vs **L8** |
|---|---|---|---|---|
| L2 비루프 (1/4 compute) | ~3.9M | 16.72 | 16.96† | +20.5% |
| **L8 비루프 (iso-compute 준거)** | ~11.8M | 7.02 | 14.07 | — |
| naive L2×4 loop | ~3.9M | 7.29 | **14.40** | **+2.3% (루프가 오히려 빠름)** |
| + gf(2다이얼) | 3.9M | 6.83 | 13.83 | −1.7% |
| + gf+inner full | 3.9M | 5.24 | 13.49 | −4.1% |

†L2는 ~17 it/s에서 eager≈compile — dataloader 한계 도달로 보임(GPU 하한 아님).
(주의: gpu1-3에서 W9 훈련 3개 동시 진행 중의 측정 — 절대값은 node2 완전유휴 벤치보다 ~6% 낮으나
단일세션 내 상대비교는 유효, node2 수치와 패턴 일치.)

- **루프 자체는 공짜(오히려 +2~4% 빠름)**: L2×4가 L8보다 빠른 건 공유 가중치 2벌만 캐시에 상주
  (L8은 8벌) → weight 읽기 트래픽 감소로 해석. "looping의 wall-clock 비용" 자체가 없음.
- **방법의 진짜 가격 (iso-compute 준거 L8 대비, compile)**: gf **−1.7%**, gf+inner **−4.1%**.
- 품질×비용 통합 (d256, PSNR은 3-seed 또는 [s95]): L8 22.098 / naive loop 22.117(동급, 파라미터 1/3.5)
  / gf 22.663 (**L8 +0.57, 처리량 −1.7%**) / gf+inner [22.864] (**L8 +0.77†, 처리량 −4.1%**) †s95, 3-seed 진행 중.
  → **"L8보다 +0.6~0.8dB 좋고 2~4% 느리고 파라미터 1/3": 방법의 최종 가격표.**

### W9 종합 (node1, 2026-07-23 새벽) — d512 풀스택 + qkv-only의 정직한 seed 판정

**d512 풀스택**: r25_d512_gfinner_lr128_s95 **+0.767** (24.551) vs d512 gf@128 +0.703 → inner 가산
+0.064 (d256의 +0.091과 일관 — inner 가산은 스케일 유지, 단일 seed).

**gf_inner full 3-seed 확정 (d256)**: +0.660/+0.551/+0.634 → **평균 +0.615, 3/3 양성**.
gf 대비 per-seed 가산: +0.091/+0.023/+0.094 → **평균 +0.069, 3/3 양성** — inner full은 seed-안정.

**qkv-only 가산은 seed-불안정 (판정 번복)**: gf 대비 +0.077/+0.069/**−0.088** → 평균 +0.019, 2/3.
s95 단독으론 "qkv만으로 충분"이었으나 3-seed에서 무너짐 → **NVS 미니멀 축소(5→2사이트)는 보류,
NVS는 full inner 유지**. 반면 LM은 qkv-only 2-seed ±0.02로 매우 안정(55.81/55.79) — 사이트 축소
가능 여부도 태스크 의존. (교훈 재확인: 단일 seed 가산 +0.08은 판정 불가.)

### W11 — L1×8 극한 케이스, LM쪽 완성 (node2): **사용자 가설 지지**

| LM 16M급 (s95) | L2×4 (naive 74.45) | L1×8 (naive 86.73) |
|---|---|---|
| naive | 74.45 | 86.73 (다양성 손실 +12.3) |
| 3다이얼 | 59.14 (회복 −15.31) | 64.02 (**회복 −22.71**) |
| +qkv-inner | 55.81 (회복 −18.64) | 61.43 (**회복 −25.30**) |

**다이얼 회복폭이 L1×8에서 ~1.4배 큼** — 고유 layer가 1개라 depth 다양성이 전부 다이얼 몫이 되는
극한에서 방법의 기여가 커짐 = "다이얼 = 잃어버린 layer-다양성의 파라미터 압축 복원" 서사 지지.
단 절대 성능은 L2×4가 우위(55.2 < 61.4) — 고유 layer 2개의 다양성은 다이얼로 완전 대체는 안 됨.
NVS쪽(r26 3런) 진행 중.

### W13 — 대형 LM 절대 처리량 (d768/seq4096/bs8 실런 설정, eager, node2 단독 GPU)

| 모델 | params | ms/step | tok/s | vs 12L 고유 |
|---|---|---|---|---|
| **orig 12L (iso-compute 준거)** | 139.6M | 160.6 | 203,990 | — |
| 3L (1/4 compute) | 71.8M | 67.3 | 486,931 | +139% |
| naive 3L×4 | 71.8M | 159.5 | **205,390** | **+0.7% (동일)** |
| + 3다이얼 | 71.8M | 179.4 | 182,674 | −10.5% |
| + 다이얼+inner | 72.0M | 199.6 | 164,203 | −19.5% |

- **"루프 = iso-compute"가 대형에서도 정확히 성립** (205.4k ≈ 204.0k tok/s).
- eager 다이얼 비용: d256 −6~11%/−28% → d768 −11%/−20% — inner는 예측대로 감소, 다이얼은 유사
  (다이얼 비용은 O(d) bytes보다 커널 런치 고정비 성격이 강함을 시사).
- LM 파이프라인은 compile 미적용이 현 상태 — NVS에서 확인된 compile 회수(−31%→−9%)를 이식할 여지.

### W10-d512 — NVS 대형(48M L8) 절대 처리량 (node1 gpu1 단일세션, 2026-07-23)

| 모델 (d512) | eager it/s | compile it/s | compile, naive 대비 | compile, L8 대비 |
|---|---|---|---|---|
| L8 비루프 (48M, iso-compute 준거) | 4.613 | 9.203 | — | — |
| naive L2×4 (13.6M) | 4.625 | 9.287 | — | +0.9% |
| + gf | 4.020 | 9.143 | **−1.6%** | **−0.7%** |
| + gf+inner full | 3.118 | 8.150 | −12.2% | −11.4% |

- **d512 compile에서 gf(2다이얼)는 사실상 공짜** (−1.6%) — d256(−4~8%)보다 개선. 스케일이 클수록
  다이얼 상대비용 감소 예측이 compile에서 확인됨.
...
### W7 — 3다이얼+inner full, 대형 LM 3B tokens 완주 (node2, 2026-07-23)

| d768 3L×4, 3B tokens | ppl |
|---|---|
| naive | 21.322 |
| 3다이얼 (lr16) | 20.854 |
| **3다이얼 + inner full (lr16)** | **20.854** |

**대형 LM에서 inner 가산 = 0.000** (20.8541 vs 20.8543). 16M에서 −3.94였던 inner 가산이 3B-토큰
d768에서 완전 소멸 — LM 조건화 이득의 스케일 축소 법칙이 inner에도 그대로 적용됨. 다이얼 자체의
−2.2%(naive 대비)는 유지. **LM 대형에선 "다이얼만"이 결론, inner는 소형 LM·NVS(d512 +0.079
seed-stable)에서만 유효 — 태스크·스케일 의존 명시.**

### W11 — L1×8 극한 케이스, NVS쪽 완성 (s95): **inner가 극한에서 3.6× 커짐**

| NVS d256 (s95) | L2×4 | L1×8 |
|---|---|---|
| naive | 22.204 | 22.007 (−0.197) |
| + gf | +0.569 | +0.484 |
| + gf+qkv-inner | +0.646 | **+0.764** |
| inner의 가산 (qkv − gf) | +0.077 | **+0.280 (3.6×)** |

- **사용자 가설 확인 — 단 주역은 gf가 아니라 inner**: 고유 layer 1개 극한에서 "공유 행렬의 loop별
  재표현"(inner) 기여가 3.6× 증폭. gf(잔차축 조건화)는 오히려 소폭 감소 — 다양성 결핍을 채우는 건
  연산자 재표현 축임. LM쪽(회복 1.4×)과 방향 일치.
- NVS의 다양성 손실은 작음(naive −0.197; LM은 +12.3 ppl) → NVS는 L1×8이 실용 후보:
  **L1×8+gf+qkv = 22.772 @ 2.47M params** — L2×4 naive(3.9M)보다 +0.57, L8 비루프(11.8M) 대비
  +0.82를 파라미터 1/4.8로. 극한 압축 스토리 성립 (s95 단일 — 3-seed 승격 착수).

### W14 — d512 풀스택 3-seed 확정: +0.767/+0.642/+0.759 → **평균 +0.723, 3/3 유의**
gf@128 3-seed +0.644 대비 inner 가산 per-seed +0.064/+0.060/+0.113 → **평균 +0.079, 3/3 양성 —
NVS의 inner full 가산은 d256(+0.069)·d512(+0.079) 모두 seed-stable.** (LM 3B의 0.000과 대조.)

### W15 — L1×8 3-seed 확정 (2026-07-23): 극한압축 후보 성립, inner 증폭 seed-안정

| L1×8 (d256) | s95 | s96 | s97 | 3-seed 평균 |
|---|---|---|---|---|
| naive (vs L2×4 naive) | −0.197 | +0.065 | −0.074 | **−0.069 (≈동급)** |
| + gf (vs L1×8 naive) | +0.484 | +0.451 | +0.608 | +0.514 |
| + gf+qkv (vs L1×8 naive) | +0.764 | +0.552 | +0.680 | **+0.665** |
| **inner(qkv) 가산** | +0.280 | +0.101 | +0.072 | **+0.151 (3/3 양성)** |

- **inner 증폭 3-seed 확정**: L2×4에서 qkv-inner 가산은 +0.019(seed-불안정)였는데 **L1×8에선
  +0.151, 3/3 양성** — s95의 3.6×는 seed 운이었지만, "고유 layer가 줄수록 inner가 커지고
  안정화된다"는 방향은 3-seed에서 성립. (다양성 결핍 → 연산자 재표현 축의 가치 증가.)
- **극한압축 확정**: L1×8+gf+qkv 3-seed **22.714 @ 2.47M** = L2×4 풀스택(22.732 @ 3.94M)과 동급,
  L8 비루프(22.098 @ 11.8M) +0.62를 파라미터 1/4.8로. naive L1×8이 L2×4와 동급(−0.07)인 것도
  NVS 특이점 (LM은 +12.3 ppl 손실).

...
def _handle_nccl_error(error, attempt, max_retries):
    """Enhanced NCCL error handling with memory cleanup"""
    error_str = str(error).lower()
    
    # Check for memory-related errors
    is_memory_error = any(keyword in error_str for keyword in [
        'out of memory', 'cuda failure 2', 'memory', 'allocation'
    ])
    
    # Check for communication errors
    is_comm_error = any(keyword in error_str for keyword in [
        'nccl error', 'connection refused', 'socket', 'communication'
    ])
    
    if is_memory_error:
        logger.warning(f"Memory-related NCCL error detected: {error}")
        _clear_memory_cache()
        return True  # Retry for memory errors
    elif is_comm_error:
        logger.warning(f"Communication NCCL error detected: {error}")
        return attempt < max_retries - 1  # Retry communication errors
    else:
...
class CheckpointManager:
    ...
    def save(
        ...
        """
        Enhanced checkpoint saving with memory management and NCCL resilience
        """
        # Log initial memory state
        initial_memory = _get_memory_stats()
        logger.info(f"Starting checkpoint save - Memory: {initial_memory}")
        ...
        # Pre-checkpoint memory cleanup
        _clear_memory_cache()
        
        logger.info("Saving...")
        
        # Enhanced retry logic with progressive recovery strategies
        max_retries = 5
        retry_delays = [5, 10, 20, 30, 60]  # Progressive backoff
        
        state_dict = None
        for attempt in range(max_retries):
            try:
                # Memory-aware state dict creation
                if state_dict is None:
                    # Clear cache before creating state dict
                    _clear_memory_cache()
                    logger.info(f"Creating state dict (attempt {attempt + 1})")
                    state_dict = self.get_state_dict(model, optimizer)
                    
                    # Log memory after state dict creation
                    post_statedict_memory = _get_memory_stats()
                    logger.info(f"State dict created - Memory: {post_statedict_memory}")
                ...
                if any(keyword in error_str for keyword in ['out of memory', 'cuda failure 2', 'memory']):
                    logger.warning(f"Memory error during checkpoint save (attempt {attempt + 1}/{max_retries}): {e}")
                    ...
                    logger.info(f"Waiting {wait_time}s for memory recovery...")
                    time.sleep(wait_time)
                    
                    if attempt < max_retries - 1:
                        continue
                
                # NCCL communication error handling
                elif any(keyword in error_str for keyword in ['nccl error', 'connection refused', 'socket']):
                    logger.warning(f"NCCL error during checkpoint save (attempt {attempt + 1}/{max_retries}): {e}")
                    
                    # Synchronize ranks and wait
                    if dist.is_initialized():
                        try:
                            dist.barrier(timeout=60)
                        except Exception as barrier_error:
                            logger.warning(f"Barrier failed: {barrier_error}")
                    
                    wait_time = retry_delays[min(attempt, len(retry_delays) - 1)]
                    logger.info(f"Waiting {wait_time}s for NCCL recovery...")
                    time.sleep(wait_time)
                    
                    if attempt < max_retries - 1:
                        continue
                
                # If we reach here, either it's the last attempt or an unrecoverable error
                logger.error(f"Failed to save checkpoint after {attempt + 1} attempts: {e}")
                ...
                if attempt == max_retries - 1:
                    raise Exception(f"Checkpoint save failed after {max_retries} attempts. Last error: {e}")
        ...
        logger.info(f"Checkpoint save completed - Final memory: {final_memory}")
        return True

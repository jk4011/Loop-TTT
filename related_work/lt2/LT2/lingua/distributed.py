...
class EnvironmentArgs:
    ...
    # PyTorch intra-node CUDA comm; leave off on some stacks (NCCL init / IntraNodeComm errors).
...
def setup_torch_distributed(dist_args):
    ...
    Initialize the following variables:
        ...
        - world_size
    """
    print("Setting up torch distributed...")
    try:
        print(f"Setting start method to {dist_args.spawn_method}...")
...
def parallelize_model(
    ...
    logger.info(f"param_dtype: {param_dtype}")
    if (
        distributed_args.fsdp_type == "full_shard"

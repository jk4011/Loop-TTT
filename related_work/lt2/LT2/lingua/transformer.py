...
@dataclass
class FlashAttnMask:
    """Attention mask specification for ``attn_impl=\"flash_attn3\"``.

    On CUDA, uses ``flash_attn.cute.flash_attn_func`` when installed. Otherwise falls back
    to :func:`torch.nn.functional.scaled_dot_product_attention`.

    Sliding-window causal matches xFormers
    ``LocalAttentionFromBottomRightMask(window_left=left, window_right=right)``:
    query position ``i`` may attend to key positions ``k`` with
    ``i - left <= k <= i + right`` (and causal masking when ``causal`` is True).
    ``window_size=None`` means full causal attention.
    """
...
_fa3_flash_attn_func: Optional[Callable[..., Any]] = None
_fa3_import_attempted: bool = False


def _get_fa3_flash_attn_func() -> Optional[Callable[..., Any]]:
    """Return ``flash_attn.cute.flash_attn_func`` if available, else ``None``."""
    global _fa3_flash_attn_func, _fa3_import_attempted
    if not _fa3_import_attempted:
        _fa3_import_attempted = True
        try:
            from flash_attn.cute import flash_attn_func as _flash_attn_func

            _fa3_flash_attn_func = _flash_attn_func
        except ImportError:
            _fa3_flash_attn_func = None
    return _fa3_flash_attn_func


def _sdpa_flash_attn3_fallback(
    xq: torch.Tensor,
    xk: torch.Tensor,
    xv: torch.Tensor,
    mask: Optional[Union[FlashAttnMask, str]],
) -> torch.Tensor:
    """``flash_attn3`` path via SDPA; inputs/outputs are ``B S H D``."""
    xq_bhsd, xk_bhsd, xv_bhsd = map(lambda t: t.transpose(1, 2), (xq, xk, xv))
    if mask is None:
        ...
        if not mask.causal:
            raise NotImplementedError(
                "FlashAttnMask with causal=False is not supported for flash_attn3"
            )
        if mask.window_size is None:
            ...
            win_left, win_right = mask.window_size
            s = xq_bhsd.size(2)
            q_idx = torch.arange(s, device=xq_bhsd.device, dtype=torch.long)[:, None]
            k_idx = torch.arange(s, device=xq_bhsd.device, dtype=torch.long)[None, :]
            allowed = (k_idx <= q_idx) & (k_idx >= q_idx - win_left) & (
                k_idx <= q_idx + win_right
            )
            out = F.scaled_dot_product_attention(
                xq_bhsd,
                xk_bhsd,
        ...
        raise NotImplementedError(
            f"flash_attn3 expects mask None or FlashAttnMask, got {type(mask)}"
        )
...
class Attention(nn.Module):
    ...
    def forward(
        ...
        mask: Optional[Union[BlockMask, AttentionBias, str, FlashAttnMask]] = None,
        attn_impl: str = "sdpa",
    ) -> torch.Tensor:
        # B S D
        ...
        if attn_impl == "flex_attention":
            ...
            # flash_attn.cute expects (batch, seqlen, nheads, headdim); see flash_attn/cute/tests.

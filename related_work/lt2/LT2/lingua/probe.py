...
def _get_stats(x_: torch.Tensor, remove_inf=False, compute_expensive=True) -> Dict[str, Any]:
    ...
    # Skip expensive statistics if memory constrained
    if compute_expensive:
        stats["skew"] = (((x - mean) / std) ** 3).double().mean()
        stats["kurtosis"] = (((x - mean) / std) ** 4).double().mean()
        # Note: `quantile` takes at most 2**24 elements, see
        # https://github.com/pytorch/pytorch/issues/64947
        stats["quantiles"] = torch.quantile(x[: 2**24], quantiles)
    return stats
...
class AutoProbeD(TorchDispatchMode):
    ...
    def log_tensor(self, name: str, x: torch.Tensor, **kwargs) -> None:
        # Disable expensive stats (skew/kurtosis/quantiles) to save memory
        kwargs.setdefault('compute_expensive', False)

...
class TrainArgs:
    ...
    model: LoopedWindowTransformerArgs = field(default_factory=LoopedWindowTransformerArgs)
...
def validate_train_args(args: TrainArgs, output_size: int):
    ...
    # Validate looping and attention pattern
    logger.info(f"Model configuration:")
    logger.info(f"  - Loop count: {args.model.loop_count}")
    logger.info(f"  - Attention pattern: {args.model.attention_pattern}")
    logger.info(f"  - Physical layers: {args.model.n_layers}")
    eff_depth = args.model.n_layers * args.model.loop_count
    logger.info(f"  - Effective depth: {eff_depth}")
...
@torch.no_grad()
def _clip_and_step(model, optimizer, scheduler, max_norm: float) -> float:
...
def train(args: TrainArgs):
    with ExitStack() as context_stack:
        ...
        with torch.device("meta"):
            model = LoopedWindowTransformer(args.model)
        ...
        while train_state.step < args.steps:
            ...
            if (args.probe_freq is not None) and every_n_steps(
                ...
                with probe:
                    ...
                    # Looped models use even more memory (3x loops), so reduce further
                    probe_bsz = max(1, bsz // 4)  # Use 1/4 batch size for looped models
                    probe_seq = seqlen // 2  # And also reduce sequence length
                    probe_loss = model(
            ...
            if train_state.acc_step == 0:
                grad_norm = _clip_and_step(
                    model, optimizer, scheduler, float(args.optim.clip)
                )
                train_state.step += 1
            ...
            if every_n_steps(
                ...
                n_layer_passes = args.model.n_layers * args.model.loop_count
                FLOPS = (
                    get_num_flop_per_token(
                ...
                # Add loss vs tokens metric for wandb custom x-axis plotting
                metrics["loss/out_vs_tokens"] = metrics["loss/out"]

                # Log iteration weights for visualization (similar to looped_linear_attention)
                base_model = model.module if hasattr(model, 'module') else model
                if hasattr(base_model, '_fsdp_wrapped_module'):
                    base_model = base_model._fsdp_wrapped_module
                
                # Helper to gather DTensor parameters
                def gather_param(param):
                    """Collect a (potentially sharded) parameter for logging.

                    Use all_gather_object to avoid shape agreement requirements; this
                    keeps GPU memory usage minimal and sidesteps empty-shard mismatches.
                    """

                    # Move to CPU for cheap serialization; iteration_weights is tiny.
                    if isinstance(param, DTensor):
                        param_local = param.to_local().detach().cpu()
                    else:
                        param_local = param.detach().cpu()
                    ...
                    if world_size > 1:
                        ...
                        torch.distributed.all_gather_object(gathered, param_local)
                        tensors = [t for t in gathered if t.numel() > 0]
                        if len(tensors) == 0:
                ...
                # Log iteration weights (learnable parameters for weighted sum)
                if hasattr(base_model, 'iteration_weights') and base_model.iteration_weights is not None:
                    iter_w = gather_param(base_model.iteration_weights)
                    if iter_w.numel() > 0:
                        iter_w = iter_w.flatten()
                        for i in range(min(iter_w.shape[0], args.model.loop_count)):
                            metrics[f"loop/iter_w_{i}"] = iter_w[i].item()
                ...
                if args.async_eval_gpus is None:
                    ...
                    with clean_env():
                        launch_job(
                            StoolArgs(
                                asdict(eval_args),
                                script="apps.LT2.eval",
                                copy_code=False,
                                nodes=max(1, (args.async_eval_gpus + 7) // 8),
                                ngpu=min(args.async_eval_gpus, 8),
                                qos="",
                                dirs_exists_ok=True,
                            )
                        )
...
def main():
    ...
    class DummyArgs:
        name: str
        model: LoopedWindowTransformerArgs

    @dataclass
    class LoopedWindowTransformerArgs:
        dim: int

    Then you can pass model.dim=32 to change values in LoopedWindowTransformerArgs

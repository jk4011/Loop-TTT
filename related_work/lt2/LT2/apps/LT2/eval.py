...
import torch
from apps.LT2.generate import (
    LoopedWindowGenerator,
...
class EvalHarnessLM(LM):
    ...
    def generate_until(self, requests: List[Instance]) -> List[str]:
        # Group requests by generation arguments
        grouped_requests = defaultdict(list)
        for i, req in enumerate(requests):
            prompt, gen_args = req.args
            # Create hashable key from gen_args (convert lists to tuples)
            key = tuple(sorted(
                (k, tuple(v) if isinstance(v, list) else v) 
                for k, v in gen_args.items()
            ))
            grouped_requests[key].append((i, prompt, gen_args))
        
        # Process each group and collect results
        results = [None] * len(requests)
        for key, group in grouped_requests.items():
            indices, prompts, gen_args_list = zip(*group)
            gen_args = gen_args_list[0]  # All same in group
            ...
            # Filter and place results in correct positions
            for idx, g in zip(indices, generations):
                for e in until:
                    g = g.replace(e, "")
                results[idx] = g
...
def launch_eval(cfg: EvalArgs):
    ...
    model.eval()
    generator = LoopedWindowGenerator(cfg.generator, model, tokenizer)
    ...
    if get_global_rank() == 0:
        # Filter out non-serializable objects (like functions) from results
        ...
        logger.info(f"All evaluation results: {serializable_results.get('results', serializable_results)}")
    ...
    if cfg.metric_log_dir and get_global_rank() == 0:
        ...
        # Use serializable_results which has already been filtered
        results_to_log = serializable_results.get("results", serializable_results)
        print(
            json.dumps(timestamp | results_to_log),

...
class DinoModelWrapper(nn.Module):
    ...
    def __init__(self, dino_model="dinov2_vitb14_reg", device="cpu"):
        ...
        embed_dim = 768 if dino_model == "dinov2_vitb14_reg" else 384
        self.classifier = nn.Linear(embed_dim, 150)
    ...
    def classify(self, x: torch.Tensor) -> torch.Tensor:
        """"""
        x = self.dino.norm(x)
        x_patch = x[:,5:] # B x 256 x 768
        logits = self.classifier(x_patch)
        logits = torch.transpose(logits, 1, 2) # B x 150 x 256
        return logits.reshape(logits.size(0), 150, 37, 37)

...
from paths import PRJ_DIR, MODEL_DIR, IMAGENET_TRAIN_DIR as IMAGENET_PATH, IMAGENET_VAL_DIR as IMAGENET_VAL_PATH

def main():
    parser = argparse.ArgumentParser(description="Train Raptor/Dino Probe on ImageNet")
    ...
    parser.add_argument("--classifier_init", type=str, default="dinov2_vitb14_reg4_linear_head.pth", help="Path to initial classifier weights (optional, only for Raptor)")
    ...
    if args.variant.startswith("raptor"):
        ...
        model = RaptorWrapper(model, dino, classifier_init_path=args.classifier_init)
    ...
    run_train(model, device, IMAGENET_PATH, IMAGENET_VAL_PATH, classifier_save_path, args.seed, args.variant, model_seed=args.model_seed)

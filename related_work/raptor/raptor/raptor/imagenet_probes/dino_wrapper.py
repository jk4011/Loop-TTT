...
class DinoModelWrapper(nn.Module):
    ...
    def __init__(self, dino_model="dinov2_vitb14_reg", device="cpu"):
        super().__init__()

        self.device = torch.device(device)
        self.dino = self._load_dino(dino_model).eval().to(self.device)
        classifier_path = "dinov2_vitb14_reg4_linear_head.pth" if dino_model == 'dinov2_vitb14_reg' else "dinov2_vits14_reg4_linear_head.pth"
        classifier = torch.load(classifier_path)
        weight = classifier['weight']
        bias = classifier['bias']
        dim = 786 if dino_model == 'dinov2_vitb14_reg' else 384
        self.classifier = nn.Linear(dim * 2, 1000)
        self.classifier.weight = nn.Parameter(weight)
        self.classifier.bias = nn.Parameter(bias)

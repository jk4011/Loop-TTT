...
def build_optimizer(model, args):
    decay, no_decay, loopp = [], [], []
    mult = getattr(args, "loop_param_lr_mult", 0.0)
    ...
    if loopp:
        groups.append({"params": loopp, "weight_decay": 0.0, "lr": args.lr * mult})
        print(f"[optzone] {len(loopp)} dial params -> wd=0, lr x{mult}", flush=True)
